...
def _neoserra_cards(soup: BeautifulSoup, source: dict) -> list[dict]:
    """The eCenterDirect front end: <div class="event-card">.

    Carries a fee flag ("No Fee") and a format line on the card itself, which
    is why these are the most trustworthy rows in the whole feed: the host
    states the price rather than leaving it to be inferred.
    """
    out = []
    for card in soup.select("div.event-card"):
        ...
        if m:
            ...
            m = re.search(r"\d+[^,]{2,40},\s*[A-Za-z .]+\s+[A-Z]{2}\s*\d{5}", blob)
            if m:
                loc = m.group(0)
...
def _neoserra_aspx(soup: BeautifulSoup, source: dict) -> list[dict]:
    """The older events.aspx front end: <div class="cdevent">.

    The date arrives split across three elements and the time lives on its own
    line, so they are stitched back together here rather than pattern-matched
    out of the surrounding text.

    Note what is NOT on these cards: a price. eCenterDirect states "No Fee"
    outright, this one says nothing, so cost comes back empty rather than being
    assumed free. Anything downstream that promises no-cost events has to treat
    an empty cost as unknown, not as zero.
    """
    out = []
    for card in soup.select("div.cdevent"):
        title_el = card.select_one(".cdeventtitle")
        if not title_el:
            continue
        title = " ".join(title_el.get_text(" ", strip=True).split())

        blob = " ".join(card.get_text(" ", strip=True).split())
        if re.search(r"on[- ]demand", blob, re.I):
            continue

        def grab(css: str) -> str:
        ...
        if href.startswith("http"):
            ...
            url = base.group(0) + "/" + href.lstrip("/")
        else:
            url = href

        loc = grab(".cdeventlocation") or grab(".cdworkshoploctitle")
        topic = grab(".cdeventtopic").replace("Topic:", "").strip()
...
def from_neoserra(soup: BeautifulSoup, source: dict) -> list[dict]:
    """Neoserra, the booking platform most state SBDCs run on.

    There are two front ends in the wild and a state can be on either, so both
    are tried and whichever matches wins. The old scan of every tr and li is
    kept last as a fallback: it found nothing on either front end, which is why
    forty-one states returned zero events while their pages loaded perfectly.
    """
    out = _neoserra_cards(soup, source) + _neoserra_aspx(soup, source)
    if out:
        return out
    ...
    for row in soup.select("tr, .event, .eventItem, li"):
        ...
        if not link:
            continue
        href = link["href"]
        if not re.search(r"/events?/\d+|eventId=|confId=|ekey=", href, re.I):
            continue
        title = " ".join(link.get_text(" ", strip=True).split())
        if len(title) < 8:
            continue
        ...
        if not is_real_title(title):
            continue
        out.append({
            "title": title, "host": source["name"], "start": m.group(0),
            "duration": "", "topic": "", "url": url, "summary": text[:400],
        })
    return out
